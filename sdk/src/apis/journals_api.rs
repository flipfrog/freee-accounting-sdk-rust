...
pub async fn get_journals(configuration: &configuration::Configuration, download_type: &str, company_id: i32, encoding: Option<&str>, visible_tags_left_square_bracket_right_square_bracket: Option<Vec<String>>, visible_ids_left_square_bracket_right_square_bracket: Option<Vec<String>>, start_date: Option<&str>, end_date: Option<&str>) -> Result<crate::models::JournalsResponse, Error<GetJournalsError>> {
    ...
    if let Some(ref local_var_str) = encoding {
        local_var_req_builder = local_var_req_builder.query(&[("encoding", &local_var_str.to_string())]);
    }
    local_var_req_builder = local_var_req_builder.query(&[("company_id", &company_id.to_string())]);
