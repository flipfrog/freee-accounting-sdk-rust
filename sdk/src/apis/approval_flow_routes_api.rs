...
pub async fn get_approval_flow_route(configuration: &configuration::Configuration, id: i32, company_id: i32) -> Result<crate::models::ApprovalFlowRouteResponse, Error<GetApprovalFlowRouteError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/1/approval_flow_routes/{id}", local_var_configuration.base_path, id=id);
...
pub async fn get_approval_flow_routes(configuration: &configuration::Configuration, company_id: i32, included_user_id: Option<i32>, usage: Option<&str>, request_form_id: Option<i32>) -> Result<crate::models::ApprovalFlowRoutesIndexResponse, Error<GetApprovalFlowRoutesError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/1/approval_flow_routes", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if let Some(ref local_var_str) = request_form_id {
        local_var_req_builder = local_var_req_builder.query(&[("request_form_id", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
