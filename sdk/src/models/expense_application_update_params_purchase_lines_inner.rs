...
pub struct ExpenseApplicationUpdateParamsPurchaseLinesInner {
    ...
    /// 補足資料（配列）   receipt_id（証憑ファイル）を指定してください。   receipt_id（証憑ファイル）は5個まで指定できます
    #[serde(rename = "sub_receipt_ids", skip_serializing_if = "Option::is_none")]
    pub sub_receipt_ids: Option<Vec<i32>>,
    /// 明細行一覧（配列）
    #[serde(rename = "expense_application_lines", skip_serializing_if = "Option::is_none")]
    pub expense_application_lines: Option<Vec<crate::models::ExpenseApplicationUpdateParamsPurchaseLinesInnerExpenseApplicationLinesInner>>,
}
